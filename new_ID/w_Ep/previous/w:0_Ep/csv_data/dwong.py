...
def h4_matchup(ref, h4_expr, threshold=20, no_match_value=-9999):
    results = []
    reference = ref[0]
    
    for b_val in h4_expr:
        if len(b_val) > 0:  # Using .size instead of len() as it's more numpythonic and avoids computing len()
            distances = np.abs(b_val[:, 1] - reference)
            index = np.argmin(distances)
            if distances[index] < threshold:
                reference = b_val[index, 1]
                results.append(b_val[index, 1])
            else:
                results.append(no_match_value)
        else:
            results.append(no_match_value)
...
def prepare_data_evt(filename, st3_emcal_threshold=8):
    ...
    for i in range(len(eng)):
        evt_result=[]
        if len(seeds[i])==0:
            whole_tuple_result.append([np.full(18, -9999)])
            continue
        ...
        for label in labels_decrease[i]:
            ...
            if len(trkl_cal)==0:
                cluster_info.append(np.full(12, -9999))
            ...
            if distances[idx] <= st3_emcal_threshold:
